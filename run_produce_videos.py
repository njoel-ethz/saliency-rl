...
def main():
    ...
    path_temp = os.path.join('output', 'temp')
    ...
    after_tuning = produce_frames(os.path.join('Atari_dataset', weight_file), 'finetuned', path_indata)
...
def produce_gt(method_used, path_indata):
    ...
    for dname in list_indata:
        ...
        if len(list_frames) > 480:  # 10 seconds videos
            list_frames = list_frames[240:640]
...
def produce_example():
    obs = cv2.imread('example_frame.png')
    smap = cv2.imread('example_smap.png')
    blurred_obs = cv2.GaussianBlur(obs, (21, 21), 0)

    limit = np.array([45, 45, 45])
    out = np.where(smap < limit, blurred_obs, obs)
    cv2.imwrite('example_blurred.png', out)
